			...
			if ($result = mysqli_query($link, $sql)){
				if (mysqli_num_rows($result) > 0){
					...
					try {
						...
						if ($oldorderstatus < $Statuscode) {    //K  D1
							$sql2 = "update `orderinfo` set `order_status`='$Statuscode' ,`updatedttime`=NOW() where order_no='$Insuranceno' and sales_id='$Salesid' and person_id='$Personid' and mobile_no='$Mobileno' and member_type=$Membertype and order_trash=0";
						...
						if ($Statuscode == "I1") {
							$sql2 = "update `orderinfo` set `order_status`='$Statuscode' ,`updatedttime`=NOW() where order_no='$Insuranceno' and sales_id='$Salesid' and order_trash=0 and order_status <> 'K5'";
							mysqli_query($link,$sql2) or die(mysqli_error($link));
							$flag=1;
						}
						...
						if (($Statuscode == "K1")||($Statuscode == "K2")||($Statuscode == "K3")||($Statuscode == "K4")) {
							if ($oldorderstatus != "K5") {
								$sql2 = "update `orderinfo` set `order_status`='$Statuscode' ,`updatedttime`=NOW() where order_no='$Insuranceno' and order_trash=0 and order_status <> 'K5'";
								mysqli_query($link,$sql2) or die(mysqli_error($link));
								$flag=1;
							}
						}
						...
						if (($Statuscode == "K5")) {
							$sql2 = "update `orderinfo` set `order_status`='$Statuscode' ,`updatedttime`=NOW() where order_no='$Insuranceno' and order_trash=0";
							mysqli_query($link,$sql2) or die(mysqli_error($link));
							$flag=1;
						}
						if ($flag == 1){
